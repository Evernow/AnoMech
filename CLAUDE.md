...
- **Only safety is tested automatically.** `tests/SafetyTests` covers the packet filter, the zone session and every path that can move the real character or talk to the server (see its README; it runs in CI on every push and PR, and daily against new Dalamud builds, whose behaviour its contract tests check the fakes against). Build the plugin first: `dotnet build AnoMech.sln -c Release -p:Platform=x64`, then `dotnet test tests/SafetyTests -c Release`. `tests/SecurityTests` covers the relay transport. Everything else is verified by "build clean → load DLL via Dalamud Dev Plugins → run a scenario in-game and watch." For UI / behavior changes, ask the user to run the plugin; you cannot. These tests are not to be fully trusted, you should always verify that no changes made can cause the server to receive information from the client that would otherwise not be possible. 
...
- **The real character only moves during a stay.** `SimPlayer` always resolves to the live character and the guard only runs during a stay, so nothing stops a move outside one at runtime. Code that can run outside a stay (settings panels, timers, callbacks) must not reach `SetPosition` / `SetRotation` / `CarryTo` at all: `StayBoundaryTests` fails the PR and prints the path. Do such work from the scenario's timeline.
